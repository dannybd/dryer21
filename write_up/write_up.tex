...
\section{Comparison to other services}
...
For each comparison a rating of \checkmark indicates that the service provides the privacy guarantee automatically.
A rating of $\sim$ indicates that the privacy guarantee is possible, but requires appropriate user action.
An empty cell indicates that the given service does not provide the given guarantee.
\begin{center}
\begin{tabular}{c|c|c|c|c}
Mixer       & \sw{Hides Participation} & \sw{Timing Obscured} & \sw{Amount Obscured} & \sw{Server Blinded} \\\hline
Bitcoin Fog & ? & \checkmark & $\sim$ &  \\\hline
BitMixer.io &  & $\sim$ & $\sim$ &  \\\hline
Dryer21     &  & $\sim$ & \checkmark & \checkmark
\end{tabular}
\end{center}
...
\item \emph{Server Blinded:} The server is no more capable of denaonymizing its users than anyone who can read the blockchain.
\end{enumerate}
Unfortunately, the precise way that Bitcoin Fog mixes its internal pool is unspecified.
